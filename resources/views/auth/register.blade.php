...
@section('content')
    <div class="flex items-center justify-center min-h-[70vh]">
        <div class="glass-panel w-full max-w-lg p-8 relative overflow-hidden">
            <div
                class="absolute top-0 right-0 w-40 h-40 bg-primary-100 rounded-full blur-3xl opacity-50 pointer-events-none">
            </div>
            <div class="absolute bottom-0 left-0 w-40 h-40 bg-teal-100 rounded-full blur-3xl opacity-50 pointer-events-none">
            </div>

            <div class="relative z-10">
                <h2 class="text-3xl font-black text-center text-slate-800 mb-2">Buat Akun Baru</h2>
                <p class="text-center text-slate-500 font-medium mb-8">Gabung ke MyBook untuk mulai meminjam</p>
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required autofocus
                            class="glass-input" placeholder="Nama Anda">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="student_id" value="{{ old('student_id') }}" required class="glass-input"
                ...
                <p class="mt-6 text-center text-sm font-medium text-slate-600">
                    Sudah punya akun? <a href="{{ route('login') }}"
                        class="text-primary-600 hover:text-primary-700 font-bold">Masuk di sini</a>
                </p>
            </div>
        </div>
    </div>
